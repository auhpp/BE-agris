...
import com.agri_supplies_shop.config.VNPConfig;
...
public class VNPayServiceImpl implements VNPayService {
    ...
    @Override
    public String createPaymentUrl(OrderRequest request, HttpServletRequest httpServletRequest) throws UnsupportedEncodingException {
        String vnp_Version = "2.1.0";
        String vnp_Command = "pay";
        String orderType = "order-type";
        String vnp_TxnRef = "";
        if (request.getVnpTxnRef() != null && !request.getVnpTxnRef().equals("")) {
            vnp_TxnRef = request.getVnpTxnRef();
        } else
            vnp_TxnRef = vnpConfig.getRandomNumber(8);
        ...
        String otherInfo = "Thanh toan don hang " + request.getAmount();
    ...
    @Override
    public void vnpayRefund(OrderRequest request, HttpServletRequest httpServletRequest) throws IOException {
        if (Objects.equals(request.getPaymentStatus(), PaymentStatus.PAID.name())
                && Objects.equals(request.getPaymentMethod(), PaymentMethod.VNPAY.name())
        ) {
            ...
            String vnp_TxnRef = request.getVnpTxnRef();
            ...
            String vnp_SecureHash = vnpConfig.hmacSHA512(vnpConfig.vnp_HashSecret, hash_Data.toString());
